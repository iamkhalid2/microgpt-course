...
The forward pass computes each node's `.data` in order:

```
Step 1:  a.data = 2.0                    (given)
Step 2:  b.data = -3.0                   (given)
Step 3:  c.data = 2.0 × (-3.0) = -6.0   (multiplication)
Step 4:  d.data = 10.0                   (given)
Step 5:  e.data = -6.0 + 10.0 = 4.0     (addition)
Step 6:  f.data = max(0, 4.0) = 4.0     (ReLU)
```
...
Each arrow is a `Value` node. Each `[×]`, `[+]`, `[relu]` is an operation that creates a new node.
...
Now that we understand what the forward pass builds, let's see how the [backward pass](./04-backward-pass.md) walks this graph in reverse to compute all the gradients.
